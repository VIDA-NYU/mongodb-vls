...
namespace mongo {
    ...
    Database::~Database() {
        ...
        log() << "Exit for database " << _name << endl;
          ...
          _collectionLock( "Database::_collectionLock" )
    {
        ...
        log() << "Init for database " << nm << endl;
        
        // initializing active mask
        activeMask = 0xFFFFFFFFFFFFFFFF; // vector of 1's
        indexActiveMask = 0xFFFFFFFFFFFFFFFF; // vector of 1's
        
        // maximum number of scans
        maxNumberScans = 64;
        
        activeMaskFull = false;
    }
    ...
    Collection* Database::getCollection( const StringData& ns ) {
        //log() << "Getting collection " << ns << endl;
