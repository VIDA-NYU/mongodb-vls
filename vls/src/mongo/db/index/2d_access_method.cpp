...
namespace mongo {
    ...
    Status TwoDAccessMethod::newCursor(IndexCursor** out, uint64_t &scanStableMask, uint64_t &scanMask) const {
        return Status(ErrorCodes::IllegalOperation, "Unimplemented seek called on S2");
    }
    
    void TwoDAccessMethod::setCollection(Collection *collection) {}
    
    void TwoDAccessMethod::addRemovedKey(const DiskLoc& loc, const BSONObj& key, const char* fieldName) {}
