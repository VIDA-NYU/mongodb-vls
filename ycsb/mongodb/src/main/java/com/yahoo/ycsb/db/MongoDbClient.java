 ...
 *
 * Modified by Fernando Chirigati on 2015 for MongoDB-VLS
 */
...
public class MongoDbClient extends DB {
    ...
    public int delete(String table, String key) {
        ...
        try {
            ...
            DBObject q = new BasicDBObject().append("_id", Integer.parseInt(key));
    ...
    public int insert(String table, String key,
            HashMap<String, ByteIterator> values, int num) {
        com.mongodb.DB db = null;
        try {
            db = mongo.getDB(database);
    ...
    public int scan(String table, String startkey, int recordcount,
        ...
        try {
            ...
            DBObject scanRange = new BasicDBObject().append("$gte", Integer.parseInt(startkey));
    ...
    @Override
    public int aggregate(String table) {
    	com.mongodb.DB db = null;
        try {
            ...
            DBObject projectValue = new BasicDBObject("key",
            		new BasicDBObject("$add", key)).append("val", new BasicDBObject("$add", val));
            
            DBObject project = new BasicDBObject("$project", projectValue);
            
            DBObject groupValue = new BasicDBObject("_id", "$key").append("count",
            		new BasicDBObject("$sum", "$val"));
            
            DBObject group = new BasicDBObject("$group", groupValue);
            
            AggregationOutput result = collection.aggregate(project, group);
            Iterable<DBObject> objects = result.results();
            Iterator<DBObject> objectsIt = objects.iterator();
            
            while (objectsIt.hasNext()) {
            	System.out.println(objectsIt.next());
            }

            return result.getCommandResult().getErrorMessage() == null ? 0 : 1;
            
            /*DBCursor cursor = collection.find();
            List<DBObject> objectList = cursor.toArray();
    ...
    @Override
    public int aggregateIndex(String table, String startkey) {
    	com.mongodb.DB db = null;
        try {
            ...
            DBObject matchValue = new BasicDBObject("_id",
            		new BasicDBObject("$gte", Integer.parseInt(startkey)));
            
            DBObject match = new BasicDBObject("$match", matchValue);
            ...
            AggregationOutput result = collection.aggregate(match, project, group);
            Iterable<DBObject> objects = result.results();
            Iterator<DBObject> objectsIt = objects.iterator();
    ...
    @Override
    public int mapReduce(String table) {
    	return 1;
    }
    
    public int commitTransaction(String table, String key,
    		HashMap<String,ByteIterator> values, HashMap<String,ByteIterator> result) {
    	return 1;
    }
