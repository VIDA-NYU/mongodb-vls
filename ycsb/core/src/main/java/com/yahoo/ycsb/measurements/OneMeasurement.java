...
public abstract class OneMeasurement {
	...
	public static HashMap<String,Integer> failures = new HashMap<String,Integer>();
	...
	public synchronized void addFailure(String name, Integer nb)
	{
		if (!failures.containsKey(name))
			failures.put(name, nb);
		else
		{
			int old_nb = failures.get(name);
			failures.put(name, nb + old_nb);
		}
	}
