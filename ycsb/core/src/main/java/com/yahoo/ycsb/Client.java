...
class ClientThread extends Thread
{
	 ...
	 */
	public ClientThread(DB db, boolean dotransactions, Workload workload, int threadid, int threadcount, Properties props, int opcount, double targetperthreadperms, int num, String indexkey)
	{
		...
		try
		{
			if (_dotransactions)
			{
				...
				while (((_opcount == 0) || (_opsdone < _opcount)) && !_workload.isStopRequested())
				{
					...
					if (!_workload.doTransaction(_db,_workloadstate, _num + _opsdone, _indexkey))
					{
						break;
					}
			...
			else
			{
				...
				while (((_opcount == 0) || (_opsdone < _opcount)) && !_workload.isStopRequested())
				{
					...
					if (!_workload.doInsert(_db,_workloadstate, _num + _opsdone))
					{
						break;
					}
...
public class Client
{
	...
	public static final String INDEX_KEY_PROPERTY="indexkey";
  ...
  public static final String MAX_EXECUTION_TIME = "maxexecutiontime";
	 ...
	 */
	private static void exportMeasurements(Properties props, int opcount, long runtime, long duration)
			throws IOException
	{
		...
		try
		{
			...
			exporter.write("OVERALL", "Duration of Main Operation(ms)", (double)duration/1000000.0);
	...
	public static void runThreads(String[] args) throws Exception
	{
		String dbname;
		...
		while (args[argindex].startsWith("-"))
		{
			...
			else if (args[argindex].compareTo("-profile")==0)
			{
				argindex++;
				Thread.sleep(15000);
			}
		...
		if (dotransactions)
		{
			...
			indexkey = props.getProperty(INDEX_KEY_PROPERTY,"0");
		}
		else
		{
			indexkey = props.getProperty(INDEX_KEY_PROPERTY,"0");
		...
		for (int threadid=0; threadid<threadcount; threadid++)
		{
			...
			Thread t=new ClientThread(db,dotransactions,workload,threadid,threadcount,props,opcount/threadcount,targetperthreadperms,num,indexkey);
			
			num += opcount/threadcount;
    ...
    long duration = 0;
		...
		for (Thread t : threads)
		{
			try
			{
				...
				duration += ((ClientThread)t).getDuration();
			}
			catch (InterruptedException e)
			{
			}
      ...
      terminator.interrupt();
		...
		try
		{
			exportMeasurements(props, opsDone, en - st, duration);
		} catch (IOException e)
		{
	...
	{
		if (args[0].compareTo("splitWorkload") == 0) {
			
			ArrayList<String> argsList = new ArrayList<String>();
			argsList.addAll(Arrays.asList(Arrays.copyOfRange(args, 1, args.length)));
			
			int splitIndex = argsList.indexOf("split");
			
			ArrayList<String> firstArray = new ArrayList<String>();
			firstArray.addAll(argsList.subList(0, splitIndex));
			
			ArrayList<String> secondArray = new ArrayList<String>();
			secondArray.addAll(argsList.subList(0, splitIndex));
			
			argsList = new ArrayList<String>(argsList.subList(splitIndex + 1, argsList.size()));
			
			splitIndex = argsList.indexOf("split");
			firstArray.addAll(argsList.subList(0, splitIndex));
			
			argsList = new ArrayList<String>(argsList.subList(splitIndex + 1, argsList.size()));

			secondArray.addAll(argsList);
			
			// starting threads
			SplitThread firstThread = new SplitThread(firstArray.toArray(new String[0]));
			SplitThread secondThread = new SplitThread(secondArray.toArray(new String[0]));
...
class SplitThread extends Thread {
	...
	public SplitThread(String[] args)
	{
		this.args = args;
	}
